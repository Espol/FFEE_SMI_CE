...
class FacturaReembolsoPdfServices implements PdfServices<FacturaReembolso> {
    ...
    @Override
    public void generarPdf(FacturaReembolso comprobante, String pathAbsolute, String numeroAutorizacion, String fechaAutorizacion, String [] sociedad, String [] documento, String porcentajeIvaDinamico) {
    	
        JasperReport jasperReport=null;
        String TMP_IREPORT = "reporte/-ruc--facturaReembolso.jasper";
        try {
        ...
        for (FacturaReembolsoDetalle fd : comprobante.getDetalles()) {
            ...
            if (fd.getDetallesAdicionales() != null) {
                for (int i = 0; i < fd.getDetallesAdicionales().size(); i++) {
                	...
                	if( "Proveedor".equals( fd.getDetallesAdicionales().get(i).getNombre() ) ){
                		row.put("Proveedor", fd.getDetallesAdicionales().get(i).getValor());
                		continue;
                	}
	    	...
	    	ReembolsoDetalle reembolsoDetalle = comprobante.getReembolsos().get(j);
	    	j++;
	    	BigDecimal baseImponibleReembolso = new BigDecimal( reembolsoDetalle.getDetalleImpuestos().get(0).getBaseImponibleReembolso() );
	        BigDecimal ivaReembolso = null;
	        
	        row.put("nroSriComprobante", reembolsoDetalle.getEstabDocReembolso() + "-" + reembolsoDetalle.getPtoEmiDocReembolso()  + "-" +  reembolsoDetalle.getSecuencialDocReembolso() );
	        row.put("baseReembolso", baseImponibleReembolso +"");// es la misma para cada impuesto
	        
	        for( DetalleImpuesto detalleImpuesto : reembolsoDetalle.getDetalleImpuestos() ) {
	        	if( ivaReembolso == null)
	        		ivaReembolso = new BigDecimal( detalleImpuesto.getImpuestoReembolso() );
	        	else
	        		ivaReembolso = ivaReembolso.add( new BigDecimal( detalleImpuesto.getImpuestoReembolso() ) );
	        }
	        
	        row.put("ivaReembolso", FormatNumberUtil.formatMilDecimal(ivaReembolso +""));// sumatoria de los impuestos
	        row.put("totalReembolso", FormatNumberUtil.formatMilDecimal(ivaReembolso.add(baseImponibleReembolso) +""));
            detalle.add(row);
