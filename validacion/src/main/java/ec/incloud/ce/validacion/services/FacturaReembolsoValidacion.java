 ...
 */
final class FacturaReembolsoValidacion implements ValidacionServices<FacturaReembolso> {

    private static ValidacionServices<FacturaReembolso> instance;
    private final String FACTURA_XSD = "/xsd/factura_1.1.0.xsd";
    private final Logger log = Logger.getLogger("integrador");

    private FacturaReembolsoValidacion() {
    }

    static ValidacionServices<FacturaReembolso> create() {
        synchronized (FacturaReembolsoValidacion.class) {
            if (instance == null) {
                instance = new FacturaReembolsoValidacion();
            }
            return instance;
        }
    }
    ...
    @Override
    public void validar(FacturaReembolso comprobante) throws ValidacionException {
        log.debug("Validar la comprobante con XSD");
        String xml = null;

        try {
            xml = XmlFactory.getFacturaReembolsoXmlServices().generarXml(comprobante);
        } catch (Exception e) {
            log.error(e);
        }
