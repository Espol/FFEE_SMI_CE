 ...
 */
class FacturaEmision implements Emision<Factura> {

    private static Emision<Factura> instance = null;
    ...
    private FacturaEmision() {
    }

    static Emision<Factura> create() {
        synchronized (FacturaEmision.class) {
            if (instance == null) {
                instance = new FacturaEmision();
            }
            return instance;
        }
    }
...
//            String montoTotal
    ...
    private void validarXML() throws ValidacionException {
        try{
            ValidacionFactory.createValidacionFacturaServices().validar(this.factura);
        }catch (ValidacionException ex){
    ...
    private void guardarDocumento(String fechaRegistro,
        ...
        documento.setRucCliente(this.factura.getInfoFactura().getIdentificacionComprador());//portal
    ...
    public String getRuc() {
    	log.warn("Ruc:" + this.factura.getInfoTributaria().getRuc());
